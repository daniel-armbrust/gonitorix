...
func createFilesystemUsage(ctx context.Context,	p *graph.GraphPeriod, devices []Device) {
	...
	for rrdFile, devs := range grouped {
		...
		if err := utils.ExecCommand(ctx, "FILESYSTEM", "rrdtool", args...); err != nil {
			logging.Error("FILESYSTEM", "Failed to create disk usage graph '%s': %v", graphFile, err,)
			continue
		}

		logging.Info("FILESYSTEM", "Created disk usage graph '%s'", graphFile,)
	}
}